 ...
 * 뒤로 위임하여, 새 소스(jira/figma 등)를 구현체 추가만으로 확장할 수 있게 한다.
 *
 * <p>1단계는 {@link ApiSpecConnector}(내부 DB 조회)만 실제 구현하고, jira/figma는 인터페이스만 열어둔다
 * (2단계 이후). 각 커넥터는 자신의 {@link #source()} 식별자를 노출하고, InvestigateLoop이 AI가 반환한
 * source 값과 대조해 해당 커넥터로 라우팅한다. 지원 커넥터가 없으면 "미지원 스킵"으로 처리된다
 * (카운터는 소비 — 카운터 우회 차단).
 *
