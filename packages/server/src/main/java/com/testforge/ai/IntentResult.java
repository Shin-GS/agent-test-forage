...
public record IntentResult(
    ...
    /** execute_recipe: 단일 레시피 실행 */
    public static IntentResult executeRecipe(Long recipeId) {
        return new IntentResult(ToolName.EXECUTE_RECIPE, null, recipeId, null, List.of(), List.of());
    }

    /** propose_plan: 여러 레시피 순차 실행 제안 */
    public static IntentResult proposePlan(List<Long> recipeIds) {
        return new IntentResult(ToolName.PROPOSE_PLAN, null, null, List.copyOf(recipeIds), List.of(), List.of());
    }

    /** select_service: 서비스 선택 요청 (유추 불가 시 빈 리스트 전달) */
    public static IntentResult selectService(List<ServiceOption> suggestedServices) {
        return new IntentResult(ToolName.SELECT_SERVICE, null, null, null, List.copyOf(suggestedServices), List.of());
    }

    /** show_candidates: 후보 레시피 목록 제시 */
    public static IntentResult showCandidates(List<RecipeCandidate> candidates) {
        return new IntentResult(ToolName.SHOW_CANDIDATES, null, null, null, List.of(), List.copyOf(candidates));
    }

    /** clarify: 모호하여 재질문 (AI 생성 메시지) */
    public static IntentResult clarify(String message) {
        return new IntentResult(ToolName.CLARIFY, message, null, null, List.of(), List.of());
    }

    /** no_match: 매칭 레시피 없음 (message는 FE 고정 문구이므로 없음) */
    public static IntentResult noMatch() {
        return new IntentResult(ToolName.NO_MATCH, null, null, null, List.of(), List.of());
    }

    /** chat: 일반 대화 응답 (AI 생성 메시지) */
    public static IntentResult chat(String message) {
        return new IntentResult(ToolName.CHAT, message, null, null, List.of(), List.of());
    }
}
