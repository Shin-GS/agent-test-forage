...
/**
 * 로그인 API. 성공 시 {@code BANK_SESSION} 쿠키를 발급한다.
 *
...
public class AuthController {
    ...
    /** 세션 쿠키 이름 */
    static final String SESSION_COOKIE = "BANK_SESSION";

    private final BankStore store;

    public AuthController(BankStore store) {
        this.store = store;
    }

    @Operation(summary = "로그인", description = "username/password로 로그인한다. 성공 시 BANK_SESSION 세션 쿠키를 발급한다. 실패 시 401.")
    @PostMapping("/login")
    ...
    /**
     * 로그인 페이지. 인증 프로필의 loginPageUrl(/login-page)이 가리키는 화면이다.
     * 데모 계정(demo/demo1234)이 미리 채워진 폼을 제공하고, [로그인] 클릭 시 POST /login을
     * 호출해 BANK_SESSION 쿠키를 발급받는다(same-origin이라 브라우저가 Set-Cookie를 자동 저장).
     */
