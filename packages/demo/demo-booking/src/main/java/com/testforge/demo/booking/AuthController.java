...
/**
 * 로그인 API. 성공 시 {@code BOOKING_SESSION} 쿠키를 발급한다.
 *
...
public class AuthController {
    ...
    /** 세션 쿠키 이름 */
    static final String SESSION_COOKIE = "BOOKING_SESSION";

    private final BookingStore store;

    public AuthController(BookingStore store) {
        this.store = store;
    }

    @Operation(summary = "로그인", description = "username/password로 로그인한다. 성공 시 BOOKING_SESSION 세션 쿠키를 발급한다. 실패 시 401.")
    @PostMapping("/login")
    ...
    /**
     * 로그인 페이지. 인증 프로필의 loginPageUrl({@code /login-page})이 가리키는 화면이다.
     * 데모 계정(demo/demo1234)이 미리 채워진 폼을 제공하고, [로그인] 클릭 시 {@code POST /login}을
     * 호출해 BOOKING_SESSION 쿠키를 발급받는다(same-origin이라 브라우저가 Set-Cookie를 자동 저장).
     */
    ...
    private static final String LOGIN_PAGE_HTML = """
            ...
            <body>
              <div class="card">
                <h1>demo-booking 로그인</h1>
                <p class="sub">예약 관리 데모 서버</p>
                <form id="loginForm">
