...
/**
 * TICKET_SESSION 쿠키를 검증하는 인터셉터. 인증이 필요한 변경 API(티켓 생성/상태변경, 코멘트 작성)에만 적용된다.
 * 조회(GET)와 CORS preflight(OPTIONS)는 항상 통과시키고, 그 외 요청만 세션을 요구한다.
...
public class SessionInterceptor implements HandlerInterceptor {
    ...
    public SessionInterceptor(TicketStore store) {
        this.store = store;
    }

    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        // 조회(GET)와 CORS preflight(OPTIONS)는 인증 대상이 아니다.
