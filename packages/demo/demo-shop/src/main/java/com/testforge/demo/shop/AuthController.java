...
public class AuthController {
     ...
     * 호출해 SHOP_SESSION 쿠키를 발급받는다(same-origin이라 브라우저가 Set-Cookie를 자동 저장).
     * 성공하면 "로그인 되었습니다"를 표시한다. 테스터가 세션을 손쉽게 확보하도록 하는 편의 화면이다.
     */
     ...
     * fetch로 {@code POST /login}(credentials: same-origin)을 호출한다. 데모 서버 편의 화면이라
     * 별도 템플릿 엔진/정적 파일 없이 컨트롤러가 직접 반환한다.
     */
