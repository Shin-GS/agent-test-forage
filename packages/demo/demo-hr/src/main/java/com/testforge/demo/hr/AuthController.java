...
/**
 * 로그인 API. 성공 시 {@code HR_SESSION} 쿠키를 발급한다.
 *
...
public class AuthController {
    ...
    /** 세션 쿠키 이름 */
    static final String SESSION_COOKIE = "HR_SESSION";

    private final HrStore store;

    public AuthController(HrStore store) {
        this.store = store;
    }

    @Operation(summary = "로그인", description = "username/password로 로그인한다. 성공 시 HR_SESSION 세션 쿠키를 발급한다. 실패 시 401.")
    @PostMapping("/login")
     ...
     * 호출해 HR_SESSION 쿠키를 발급받는다(same-origin이라 브라우저가 Set-Cookie를 자동 저장).
     */
