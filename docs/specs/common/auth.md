...
## 프로덕션 전 보안 백로그 (프로토타입에서 의도적으로 미룬 항목)

아래는 프로토타입 단계에서 **의도적으로 미룬** 항목이다. 기능/소유권 검증에는 영향이 없으나(취약점 아님), 프로덕션 배포 전에는 도입을 검토한다.

| 항목 | 현황 | 프로덕션 전 권장 |
|------|------|----------------|
| **rate limiting** | 미구현 | 로그인 무차별 대입(brute-force)·스펙 등록(`POST /specs`) 남용 방어용 도입 검토 |
| **로그인 실패 잠금** | 미구현 | 연속 실패 시 계정/IP 일시 잠금 |
| **요청 payload 크기 상한** | Spring Boot 기본값 의존 | `POST /specs`가 대형 OpenAPI 문서를 받을 수 있어 명시적 상한 설정 검토(대형 스펙 5MB+ 비동기 파싱은 [registration.md](../spec/registration.md) 백로그) |
| **선언적 입력 검증(`@Valid` + Bean Validation)** | 컨트롤러 경계 미사용, 서비스 계층에서 수동 검증 | 컨트롤러 경계에서 일관된 검증으로 누락 리스크 축소(현재도 서비스에서 검증하므로 기능 문제는 없음) |
| **로그인 타이밍 기반 계정 열거** | 존재하지 않는 아이디는 bcrypt 검증을 건너뛰어 이론상 응답 시간 차이 존재 | rate limiting과 함께 낮은 우선순위로 처리 |
