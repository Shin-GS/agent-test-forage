...
사용자 메시지 전송
    │
    ▼
BE: 컨텍스트 조립 (서비스, 레시피 목록, 이력, 지목 레시피)
    │
    ▼
...
| `propose_plan` | 플랜 제안 (결과 의존 순차 조합) | 액션 피커: 플랜 제안 카드 (스킵/순서변경/값 사전 편집) |
...
- 모든 AI 응답은 Function Calling으로 구조화된 JSON 반환
- FE는 tool 기반으로 고정 템플릿 렌더링 (clarify/chat만 AI message 사용)
- 액션 피커가 열려있으면 채팅 input 차단
- 할루시네이션 금지: 모르면 clarify, 없으면 no_match
- 빈 메시지: BE API에서 차단 (400 반환) + FE 전송 버튼 비활성화

## 서비스 외 요청 / 악용 대응

시스템 프롬프트에서 처리 (별도 호출/tool 불필요):
...
> 단, **등록된 서비스의 정책/기능 조회**(예: "이 회원가입 정책이 뭐야?")는 외부 검색이 아니라 `investigate` tool로 허용된다. (api_spec/confluence 커넥터)
