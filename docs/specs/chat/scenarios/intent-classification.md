...
- **`targetRecipeId`가 있으면 서비스가 확정이므로 `select_service`를 스킵한다.** 지목 레시피는 소속 서비스(`apiSpecId`)가 자명하므로, 대상 서비스도 사실상 확정된다 — **지목이 곧 서비스 확정**이라 서비스를 되묻는 `select_service`/`clarify` 단계가 불필요하다. BE는 지목 레시피 서비스로 대화방 서비스를 자동 설정/전환한 뒤 그 레시피를 `execute_recipe`로 실행한다(대화방 서비스 상태별 미설정/다른서비스/같은서비스 분기와 원자성은 [chat/overview.md 지목 레시피 실행 시 서비스 자동 설정/전환](../overview.md#지목-레시피-실행-시-서비스-자동-설정전환) 참조). 자유 텍스트 발화(targetRecipeId 없음)만 서비스가 불명확할 수 있어 `select_service`/`clarify`를 탄다.
...
## 결정 트리 (위에서부터 순서대로 판단)
0. 지목 레시피(targetRecipeId)가 있으면 → 그 레시피를 execute_recipe로 실행 (서비스는 지목 레시피 기준으로 확정 — select_service/clarify 스킵)
...
- targetRecipeId가 있으면 해당 레시피를 execute_recipe로 실행하세요. 이 경우 서비스는 지목 레시피 기준으로 확정되므로 select_service/clarify로 서비스를 되묻지 마세요(지목이 곧 서비스 확정). 대화방 서비스 전환은 BE가 처리합니다.
...
|------|-----------|-----------|------------|
| execute_recipe | AI message | [자동 실행] [직접 입력] [취소] | 지목 레시피 설정 (targetRecipeId면 대화방 서비스 자동 설정/전환) |
