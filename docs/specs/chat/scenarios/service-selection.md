...
사용자가 채팅에서 **자유 텍스트**로 메시지를 전송했을 때, 대화방에 서비스가 미지정인 상태.
AI가 [Tool Use 호출](intent-classification.md)에서 `select_service` tool을 선택하면 이 시나리오로 진입한다.

> **지목 레시피([▶], `targetRecipeId`) 실행은 이 시나리오를 타지 않는다.** 지목 레시피는 서비스가 자명하므로 `select_service` 없이 지목 레시피 서비스로 대화방 서비스를 자동 설정/전환한다(미설정→자동 설정, 다른 서비스→전환+SYSTEM 알림, **같은 서비스→무변경·알림 없음**). 상세: [overview.md 지목 레시피 실행 시 서비스 자동 설정/전환](../overview.md#지목-레시피-실행-시-서비스-자동-설정전환).
