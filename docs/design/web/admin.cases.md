...
| 3 | 사용자 관리 | 사용자 목록 + 역할/상태 + [+ 사용자 추가] |
| 4 | 계정 생성 | 아이디 + 비밀번호 + 역할 직접 입력 폼 |
...
- [+ 사용자 추가] 클릭 시 계정 생성 폼 노출
- 입력 필드: 아이디(text) / 비밀번호(password) / 역할(select: 일반 사용자·관리자)
- 관리자가 지정한 비밀번호로 즉시 로그인 가능 (임시 비밀번호 발급 아님)
- [생성] / [취소] 버튼. 취소 시 사용자 목록으로 복귀
- 각 입력에 label 연결(for/id)
